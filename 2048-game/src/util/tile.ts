...
let currentId = 0;
export function makeTile(tileList: TileDto[]) {
  let newTile = {} as TileDto;
...
export function moveTile({ tileList, x, y }: MoveTile) {
  assert(x === 0 || y === 0, '');
  const isMoveY = y !== 0;
  const isMinus = x + y < 0;
  const sorted: TileDto[] = tileList
  ...
  for (let i = 0; i < sorted.length; i++) {
    if (sorted[i] && sorted[i].isDisabled) {
      continue;
    }
